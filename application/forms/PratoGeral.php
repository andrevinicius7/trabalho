...
class Application_Form_PratoGeral extends Zend_Form {
        ...
        public function init() {
            $this->setMethod('post');
            $prato = new Zend_Form_Element_Text('nome' , array('label' => 'NOME','required' => true));
            $prato->addFilter(new Zend_Filter_StringToUpper());
            $this->addElement($prato);

            $categoria = new Zend_Form_Element_Select('idcategoria', array('label' => "CATEGORIA",'required' =>true));
            $this->addElement($categoria);
            
            $preco = new Zend_Form_Element_Text('preco', array('label' => "preco",'required' =>true));
